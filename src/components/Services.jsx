    ...
    <section id="services" aria-labelledby="services-title" className="bg-slate-50 py-20 transition-colors duration-300 dark:bg-[#07182d] lg:py-28">
      <div className="section-shell">
        <div className="max-w-3xl">
          <p className="eyebrow">{t.sections.services.eyebrow}</p>
          <h2 id="services-title" className="section-heading mt-5">{t.sections.services.title}</h2>
          <p className="muted-copy mt-4">{t.sections.services.description}</p>
        </div>
        <div className="mt-12 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {serviceMeta.map((service) => {
            const Icon = icons[service.icon];
            const item = t.services[service.id];
            return (
              <article key={service.id} className="card group flex min-h-[270px] flex-col p-6 hover:-translate-y-1 hover:border-sky-200 hover:shadow-glow dark:hover:border-sky-300/30">
                ...
                <h3 className="mt-5 text-xl font-black text-brand-navy dark:text-white">{item.title}</h3>

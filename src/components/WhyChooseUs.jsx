...
export default function WhyChooseUs() {
  ...
  const sec = content.sections?.why?.[language] || t.sections.why;
  const itemList = (content.whyItems || t.whyItems).map((item, index) => {
    const defaultText = t.whyItems[index] || {};
    return {
      title: item[language]?.title || defaultText.title,
      text: item[language]?.text || defaultText.text,
    };
  });
    ...
    <section className="bg-white py-20 transition-colors duration-300 dark:bg-[#10213d] lg:py-28">
      <div className="section-shell grid gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
        <div>
          ...
          <h2 className="section-heading mt-5">{sec.title}</h2>
